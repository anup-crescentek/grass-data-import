...
-------------------------
This build is gated to ONLY create/update these 3 services:
...
from the source. sports-turf-installation and new-construction-turf were stubs in
the source (hero + intro + 4 bullets only), so their body, pricing, Key Features,
and FAQ are hand-written here to match the other service pages. How It Works on
those two falls back to the shared generic steps.
...
posts appear there. The canonical set is now 10:
